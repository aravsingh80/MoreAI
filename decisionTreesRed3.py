...
with open(dataFile) as f: 
    ...
    for x in line_list:
        if count == 0:
            ...
            if '?' in lList:
                vote = ''
                party = lList[len(lList) - 1].strip()
                i = []
                i2 = 0
                for z in lList:
                    if z.strip() == '?': i.append(i2)
                    i2 += 1
                for i3 in i:
                    yV = 0
                    nV = 0
                    for z in line_list:
                        z = z[1:]
                        if z[i3].strip() == 'y' and z[len(z) - 1] == party: yV += 1
                        if z[i3].strip() == 'n' and z[len(z) - 1] == party: nV += 1
                    if yV > nV: lList[i3] = 'y'
                    else: lList[i3] = 'n'
            #if '?' not in lList: nonmissing.append(lList)
            nonmissing.append(lList)
...
print(len(nonmissing))
testSet = nonmissing[385:]
sizes = []
accuracies = []
